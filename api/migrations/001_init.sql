-- GoldenAcres API — initial schema
-- MySQL 8+ / utf8mb4. All timestamps are UTC, stored with millisecond precision.
--
-- Security notes:
--   * No plaintext secret is ever stored. Passwords use Argon2id (PHP password_hash),
--     tokens are stored only as SHA-256 hashes.
--   * Every domain row carries user_id and is filtered by it on every query.
--   * Rows are soft-deleted (deleted_at) so clients can sync deletions; a purge
--     job removes them permanently after the retention window.
